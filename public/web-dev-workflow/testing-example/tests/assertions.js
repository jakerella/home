...
QUnit.test("Assertion types", function(assert) {
    ...
    assert.deepEqual(f, { x: 1, y: 2 }, "Prototype values are included");
    
    assert.propEqual(f, { x: 1, y: 2 }, "Direct object props only, so this FAILS");
    assert.propEqual(f, { x: 1 }, "Direct properties match!");
